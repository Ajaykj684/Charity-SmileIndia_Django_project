...
def ContactPage(request):
    if request.method == 'POST':
        ...
        Contact.objects.create(user=user, name=name,
                               email=email, phone=phone, message=message)
        messages.success(
            request, 'Happy to see your interest, we will contact you soon,thank you  !')
...
def Signup(request):
    if request.method == "POST":
        ...
        User.objects.create_user(first_name=firstname, last_name=lastname,
...
def Payment(request):
    if request.method == "POST":
        ...
        place = "usa"
        Donation.objects.create(payment_id=payment_id, Category=Category,
                                user=user, amount=amount, payment_method=payment_method, place=place)
...
def Reviews(request):
    message = Message.objects.all().order_by('-id')[:4]
